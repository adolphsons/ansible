...
options:
  user:
    description:
    - Username for sftp connection. If not provided, sftp_file will try to use:
  ...
  password:
    description:
      - Password for sftp connection. If not provided, sftp_file will attempt
...
def run_sftp(module):
    ...
    if (module.params["user"]):
        ...
        try:
            ...
            try:
                user = (ConfigSectionMap("defaults")['remote_user'])
            except:
    ...
    ### If ssh_key parameter is not provided, try ansible.cfg for private_key_file value
        ...
        try:
            ssh_private_key = (ConfigSectionMap("defaults")['private_key_file'])
